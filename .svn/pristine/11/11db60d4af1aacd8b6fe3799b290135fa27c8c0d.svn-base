...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    bestGhostPositions = [beliefs.argMax() for beliefs in livingGhostPositionDistributions]
    bestPosition, bestProb = None, 0
    for ghost, ghostPos in enumerate(bestGhostPositions):
        if livingGhostPositionDistributions[ghost][ghostPos] > bestProb:
            bestPosition, bestProb = ghostPos, livingGhostPositionDistributions[ghost][ghostPos]
    bestAction, bestDistance = None, float("inf")
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition, action)
        if bestAction == None or self.distancer.getDistance(bestPosition, successorPosition) < bestDistance:
            bestAction, bestDistance = action, self.distancer.getDistance(bestPosition, successorPosition)
    return bestAction
