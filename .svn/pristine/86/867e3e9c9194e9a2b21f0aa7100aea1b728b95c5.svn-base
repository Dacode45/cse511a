  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
        ...
        for location in self.legalPositions:
            ...
            self.beliefs[location] = emissionModel[trueDistance] * self.beliefs[location]
    self.beliefs.normalize()
    "*** YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    legalLength = len(legal)
    self.particles = [ legal[i%legalLength] for i in range(self.numParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.particles:
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
        updated.append(util.sample(newPosDist))
...
class JointParticleFilter:
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.particles:
        update = []
        for i in range(self.numGhosts):
            newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldPos)), i, self.ghostAgents[i])
            update.append(util.sample(newPosDist))
        updated.append(tuple(update))
    self.particles = updated
  ...
  def observeState(self, gameState):
    ...
    newBeliefs, oldBeliefs = util.Counter(), self.getBeliefDistribution()
    for particle in self.particles:
        weight = 1.0 # may not work to set this to 1.0. Should normalize, but if all of the ghost in gail, this should force consistency.
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                ...
                # TODO: update weight with every ghost not just one
                weight *= emissionModels[i][distance]
        newBeliefs[particle] += weight
    # weights = 0
    if not any(newBeliefs.values()):
        self.initializeParticles()
        for particle in self.particles:
